...
def build_landing_story(
    ...
    card_dangerous_trace_count = len({finding.get("trace_id") for finding in card_findings if finding.get("trace_id")})
    blocked_cards = blocked_cards_from_findings(findings, demo_story, story_context)
    dangerous_trace_count = len({finding.get("trace_id") for finding in findings if finding.get("trace_id")})
...
def blocked_cards_from_findings(
    ...
    for finding in findings:
        ...
        title = titles.get(finding_type) or titles.get("default") or finding_type.replace("_", " ").title()
...
def _policy_highlights(context: dict[str, Any] | None, pack: LoadedAgentPack) -> list[dict[str, str]]:
        ...
        highlights.append(
            {
                ...
                "threshold": metric.get("threshold_display") or item.get("threshold_label", "Gate threshold"),
...
def _agent_display_name(context: dict[str, Any] | None) -> str | None:
    ...
    return profile.get("display_name") or profile.get("agent_name") or context.get("release_decision", {}).get("agent_id")
...
def _secondary_proof(context: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    if status == "verified" and blocking_failed == 0:
        verdict_note = (
            f"{agent_version} verified the controls generated by the blocked {source_version} release."
        )
        blocker_summary = "All inherited blocker controls passed."
        if has_nonblocking_warnings:
            warning_summary = (
                "Approved, not perfect — non-blocking warnings remain visible."
            )
    elif status == "not_available":
        verdict_note = (
            f"{agent_version} is APPROVED from current gate-bound blocker metrics only."
        )
    else:
...
def _featured_path_summary(finding: dict[str, Any] | None) -> str:
    _ = finding
    return (
        "A low-privilege role reached `deep_investigate_alert` "
        "in controlled release evidence."
    )
...
def _improvement_loop(
    ...
    unauthorized_attempt_rate = approved_metrics.get("unauthorized_dangerous_tool_attempt_rate", {}).get("display_value", "0.0%")
    policy_violation_rate = approved_metrics.get("dangerous_tool_policy_violation_rate", {}).get("display_value", "0.0%")
    ...
    return {
        ...
        "regression_gate_scope": loop_config.get("regression_gate_scope", "Emitted from blocked candidate bundle"),
