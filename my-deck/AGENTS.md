...
After `pnpm build`, start preview and capture all pages:

```bash
cd my-deck && npm run preview -- --host 127.0.0.1 --port 4173
# separate terminal, from repo root:
uv sync --group dev
uv run playwright install chromium
uv run python scripts/playwright_deck_shots.py --base-url http://127.0.0.1:4173
```

Screenshots land in `artifacts/deck-audit/<slide-id>/`. The script checks HTTP 200 for every `?p=` route and that `notes` count matches exported page count.
